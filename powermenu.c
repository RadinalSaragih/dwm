...
# define S_FORMAT_CLEAR "sed 's/^..//'"
	...
	if (strcmp (exit_action, S_LOCK) == 0) system ("xsecurelock");
	else if (strcmp (exit_action, S_RESTART_DWM) == 0) quit (& (const Arg) {1});
	else if (strcmp (exit_action, S_SCREEN_OFF) == 0) system ("sleep .5; xset dpms force off");
	...
	else if (strcmp (exit_action, S_CHANGE_WP) == 0) system ("RandoWall");
